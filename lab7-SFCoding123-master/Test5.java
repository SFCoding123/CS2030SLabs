...
/**
 * Test 5 for CS2030S Lab 7 (AY21/22 Sem 2).  Tests
...
class Test5 {
  ...
  public static void main(String[] args) {
    ...
    i.expect("InfiniteList.<Integer>sentinel().takeWhile(x -> x < 0).isSentinel()\n" +
        " ..returns true",
        () -> InfiniteList.<Integer>sentinel().takeWhile(x -> x < 0).isSentinel(), true);

    i.expect("InfiniteList.iterate(1, x -> x + 1).takeWhile(x -> x < 0).isSentinel()\n" +
        " ..returns false",
    ...
    i.expect("InfiniteList.iterate(1, x -> x + 1).takeWhile(x -> x < 2).isSentinel()\n" +
        " ..returns false",
