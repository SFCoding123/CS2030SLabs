class Test5 {
  ...
  public static void main(String[] args) {
    ...
    i.expectCompile("Box<Integer> b = Box.of(4).map(new AddOne()) compiles cleanly",
        "Box<Integer> b = Box.of(4).map(new Transformer<Integer,Integer>() {" +
        "  public Integer transform(Integer t) { return t; } " +
        "})",
        true);
    ...
    i.expectCompile("Box<Integer> b = Box.of(\"string\").map(new StringLength()) compiles cleanly",
        "Box<Integer> b = Box.of(\"string\").map(new Transformer<String,Integer>() {" +
    ...
    i.expect("Box.of(\"string\").map(new StringLength()).filter(new DivisibleBy(5))" +
        ".map(new AddOne()) becomes empty", 
        Box.of("string")
            .map(new StringLength())
            .filter(new DivisibleBy(5))
            .map(new AddOne()), 
          Box.empty());

    i.expect("Box.of(\"chocolates\").map(new StringLength())." +
        "filter(new DivisibleBy(5)).map(new AddOne()) becomes Box.of(11)", 
        Box.of("chocolates")
            .map(new StringLength())
            .filter(new DivisibleBy(5))
            .map(new AddOne()), 
    ...
    i.expectCompile("Box<Object> o = Box.of(4).map(new AlwaysNull()) compiles cleanly",
        "Box<Object> o = Box.of(4).map(new Transformer<Integer,Object>() {" +
    ...
    i.expectCompile("Box<Integer> b = Box.of(\"string\").map(new LastDigitsOfHashCode(2))" +
        " compiles cleanly",
    ...
    i.expectCompile("Box<B> b = Box.of(new A()).map(new AtoB()) compiles cleanly",
        "class A {}" +
        "class B {}" +
        "class AtoB implements Transformer<A,B> {" +
        "  public B transform(A t) { return new B(); }" +
        "}" +
