class Test3 {
  ...
  public static void main(String[] args) {
    ...
    i.expect("Box.<Double>of(8.8).filter(new IntValueIsPositive())" +
        ".filter(new IntValueIsPositive()) does not change",
        Box.<Double>of(8.8)
            .filter(new IntValueIsPositive())
            .filter(new IntValueIsPositive()), 
          Box.of(8.8));

    i.expect("Box.<Long>of(-100L).filter(new IntValueIsPositive())" +
        ".filter(new IntValueIsPositive()) is empty",
        Box.<Long>ofNullable(-100L)
            .filter(new IntValueIsPositive())
            .filter(new IntValueIsPositive()), 
          Box.empty());
  }
